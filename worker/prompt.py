...
\n
INSTRUCTIONS:\n
1. Summarize hard skills or requiremenets from the `Job Description`, removing extra unnecessary details, list them as bullet points, max 70 characters each. DO not write more than 6 bullet point, only includes the MOST related and important one.\n
...
9. Consider `Good Output` and `Bad Output` and `SAMPLE`  as just examples! The actual job advertisement is marked as `Real Advertisement` which you need to analyze.
10. Rewrite a summary of the `Real Advertisement` so that you can understand it better. Add your rewritten text 3 lines after the json.
"""
